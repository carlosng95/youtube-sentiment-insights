...
def load_vectorizer(vectorizer_path: str):
    ...
    try:
        ...
        logger.debug('TD-IDF vectorizer loaded from: %s', vectorizer_path)
        return vectorizer
    except Exception as e:
...
def save_model_info(run_id: str, model_path: str, file_path: str) -> None:
    try:
        ...
        logger.error('Error ocurred while saving the model info: %s', e)
        raise
...
def main():
    ...
    with mlflow.start_run() as run:
        try:
            ...
            report, cm = model_evaluation(model, X_test_tfidf, y_test)
